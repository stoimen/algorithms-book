...
[![Lowest Common in a BST](../images/4.-Lowest-Common-in-a-BST.png)](../images/4.-Lowest-Common-in-a-BST.png)In a BST we compare both values with a given node (starting from the root). In case the node’s value is in between them – this is the lowest common ancestor. If not – we go either on the left or on the right!
